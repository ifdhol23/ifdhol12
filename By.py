...
- Dol say = Mengikuti Apa Yang DiKatakan
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
				...
				if msg.from_ in admin:
					...
					kk.sendMessage(msg)
            elif msg.text in ["æ„??7?ã®ãƒ??7?ãƒ¬ã??7?¼ãƒ³ãƒ˄1?7","Gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					...
					msg.text = None
					cl.sendMessage(msg)
            elif msg.text in ["æ„??7?ã®ãƒ??7?ãƒ¬ã??7?¼ãƒ³ãƒ˄1?7","Cv1 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					ki.sendMessage(msg)
            elif msg.text in ["æ„??7?ã®ãƒ??7?ãƒ¬ã??7?¼ãƒ³ãƒ˄1?7","Cv2 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kk.sendMessage(msg)
            elif msg.text in ["æ„??7?ã®ãƒ??7?ãƒ¬ã??7?¼ãƒ³ãƒ˄1?7","Cv3 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kc.sendMessage(msg)
            elif msg.text in ["æ„??7?ã®ãƒ??7?ãƒ¬ã??7?¼ãƒ³ãƒ˄1?7","All gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
              ...
              if msg.from_ in admin:
                if wait["ProtectQR"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["é€£çµ¡å??7?˄1?7:ã‚ªãƒ1?7","K on","Contact on","é¡¯ç¤ºï¼šé–??7?1?7"]:
				if msg.from_ in admin:
					if wait["contact"] == True:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["é€£çµ¡å??7?˄1?7:ã‚ªãƒ??7?1?7","K off","Contact off","é¡¯ç¤ºï¼šé—ń1?7"]:
				if msg.from_ in admin:
					if wait["contact"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["è‡ªå??7???7?å??7?åń1?7 :ã‚ªãƒ1?7","Join on","Auto join:on","è‡ªå??7???7?åƒåń1?7 ï¼šé–??7?1?7"]:
				if msg.from_ in admin:
					if wait["autoJoin"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["è‡ªå??7???7?å??7?åń1?7 :ã‚ªãƒ??7?1?7","Join off","Auto join:off","è‡ªå??7???7?åƒåń1?7 ï¼šé—ń1?7"]:
				if msg.from_ in admin:
					...
					try:
						...
						if strnum == "off":
							...
							if wait["lang"] == "JP":
								...
								cl.sendText(msg.to,"å…³äº??7?é??7?€è¯·æ‹??7?ç»ã€‚è¦æ??7?¶å¼€è¯·æŒ‡å®šäººæ??7?°å??7?é€")
						else:
							num =  int(strnum)
							...
							if wait["lang"] == "JP":
								...
								cl.sendText(msg.to,strnum + "ä½¿äººä»¥ä¸‹çš??7?å°ç»??7?ç??7?¨è??7?ªåŠ¨é??7?€è¯·æ‹??7?ç»1?7")
					except:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"Value is wrong")
						else:
							cl.sendText(msg.to,"Bizarre ratings")
            elif msg.text in ["å¼·åˆ¶è‡ªå??7???7?é€€å??7?1?7:ã‚ªãƒ1?7","Leave on","Auto leave:on","å¼·åˆ¶è‡ªå??7???7?é€€å??7?ºï¼šé??7???7?1?7"]:
				if msg.from_ in admin:
					if wait["leaveRoom"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"è¦äº†å¼€ã€??7?1?7")
            elif msg.text in ["å¼·åˆ¶è‡ªå??7???7?é€€å??7?1?7:ã‚ªãƒ??7?1?7","Leave off","Auto leave:off","å¼·åˆ¶è‡ªå??7???7?é€€å??7?ºï¼šé??7?ń1?7"]:
				if msg.from_ in admin:
					if wait["leaveRoom"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"already")
            elif msg.text in ["å…±æœ??7?1?7:ã‚ªãƒ1?7","Share on","Share on"]:
				if msg.from_ in admin:
					if wait["timeline"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"è¦äº†å¼€ã€??7?1?7")
            elif msg.text in ["å…±æœ??7?1?7:ã‚ªãƒ??7?1?7","Share off","Share off"]:
				if msg.from_ in admin:
					if wait["timeline"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"è¦äº†å??7?³æ??7?­ã€ 1?7")
				...
				if msg.from_ in admin:
					...
					if album["result"]["items"] == []:
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"ç›¸å??7?Œæ²¡åœ¨ã€ 1?7")
					else:
						if wait["lang"] == "JP":
							mg = "The following is the target album"
						else:
							mg = "ä»¥ä¸‹æ˜¯å¯¹è±¡çš??7?ç??7?¸å??7?ń1?7"
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,str(i) + "åˆ é™¤äº??7?äº??7?çš??7?ç??7?¸å??7?Œã€ 1?7")
            elif msg.text in ["Group id","ç¾¤çµ„å??7?¨id"]:
				if msg.from_ in admin:
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,"æ‹??7?ç»äº??7?å??7?¨éƒ¨çš??7?é??7?€è¯·ã€??7?1?7")
            elif "album removeâ†??7?1?7" in msg.text:
				if msg.from_ in admin:
					gid = msg.text.replace("album removeâ†??7?1?7","")
					albums = cl.getAlbum(gid)["result"]["items"]
					i = 0
					if albums != []:
						for album in albums:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,str(i) + "åˆ é™¤äº??7?äº??7?çš??7?ç??7?¸å??7?Œã€ 1?7")
            elif msg.text in ["è‡ªå??7???7?è¿½åń1?7 :ã‚ªãƒ1?7","Add on","Auto add:on","è‡ªå??7???7?è¿½åń1?7 ï¼šé–??7?1?7"]:
				if msg.from_ in admin:
					if wait["autoAdd"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"è¦äº†å¼€ã€??7?1?7")
            elif msg.text in ["è‡ªå??7???7?è¿½åń1?7 :ã‚ªãƒ??7?1?7","Add off","Auto add:off","è‡ªå??7???7?è¿½åń1?7 ï¼šé—ń1?7"]:
				if msg.from_ in admin:
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,"doneã€??7?1?7")
            elif msg.text in ["Message","è‡ªå??7???7?è¿½åń1?7 å•å€™èªžç¢ºèª1?7"]:
				if msg.from_ in admin:
					if wait["lang"] == "JP":
						cl.sendText(msg.to,"message change to\n\n" + wait["message"])
					else:
						cl.sendText(msg.to,"The automatic appending information is set as followsã€??7?\n\n" + wait["message"])
				...
				if msg.from_ in admin:
					...
					if c in [""," ","\n",None]:
						...
						wait["comment"] = c
						cl.sendText(msg.to,"changed\n\n" + c)
            elif msg.text in ["ã‚³ãƒ¡ãƒ³ãƒ˄1?7:ã‚ªãƒ1?7","Comment on","Comment:on","è‡ªå??7???7?é¦??7?Ä1?7 ç•™è¨€ï¼šé??7???7?1?7"]:
				if msg.from_ in admin:
					if wait["commentOn"] == True:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"è¦äº†å¼€ã€??7?1?7")
            elif msg.text in ["ã‚³ãƒ¡ãƒ³ãƒ˄1?7:ã‚ªãƒ??7?1?7","Comment on","Comment off","è‡ªå??7???7?é¦??7?Ä1?7 ç•™è¨€ï¼šé??7?ń1?7"]:
				if msg.from_ in admin:
...
#------------------------------------------------------------------
				...
				if msg.from_ in admin:
					try:
						...
						cl.sendText(msg.to,"Error")
            elif "fakecâ†??7?1?7" in msg.text:
				if msg.from_ in admin:
					try:
						source_str = 'abcdefghijklmnopqrstuvwxyz1234567890@:;./_][!&%$#)(=~^|'
						name = "".join([random.choice(source_str) for x in xrange(10)])
						anu = msg.text.replace("fakecâ†??7?1?7","")
			...
			for zx in range(0,20):
				...
				if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
					try:    
						...
						kk.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Instagram : Instagram.com/ifdhol12")
						print "DiLike"
					except:
							pass
				else:
